...
$pdf->Cell(70, 5, 'EMPLEADO', 1, 0, 'C', 1);
$pdf->Cell(0, 5, 'CARGO', 1, 0, 'C', 1);
...
if (!empty(isset($_REQUEST['vdesde'])) && !empty(isset($_REQUEST['vhasta']))) {
    if ($_REQUEST['opcion'] == 1) { //por codigo
        ...
        $usuarios = consultas::get_datos("select * from v_usuarios where upper(empleado) between upper('".$_REQUEST['vdesde']."') and upper('".$_REQUEST['vhasta']."')");
    }
}else{
...
if (!empty($usuarios)) {
    foreach ($usuarios as $usu) {
        $pdf->Cell(30, 5, $usu['usu_cod'], 1, 0, 'C', 1);
        $pdf->Cell(70, 5, $usu['empleado'], 1, 0, 'L', 1);
        $pdf->Cell(0, 5, $usu['car_descri'], 1, 0, 'L', 1);
        $pdf->Ln();
    }
}else{
